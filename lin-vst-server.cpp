...
void RemoteVSTServer::eventloop()
{
 ...
 //    }      
      ...
      ymove += offset.y;
        ...
        if (hWndvst[pidx] && guiVisible && !exiting) 
	{
#ifdef XOFFSET	
        MoveWindow(hWndvst[pidx], GetSystemMetrics(SM_XVIRTUALSCREEN) + xmove, GetSystemMetrics(SM_YVIRTUALSCREEN) + ymove, width + width2, height, true);
#else	   
	MoveWindow(hWndvst[pidx], GetSystemMetrics(SM_XVIRTUALSCREEN) + xmove, GetSystemMetrics(SM_YVIRTUALSCREEN) + ymove, width, height, true);
#endif
       }
        break;
...
#ifdef EMBED
#ifdef VST32SERVER
  cerr << "LinVst-X version 4.9-32bit" << endl;
#else
  cerr << "LinVst-X version 4.9-64bit" << endl;
#endif
#else
#ifdef VST32SERVER
  cerr << "LinVst-X version 4.9st-32bit" << endl;
#else
  cerr << "LinVst-X version 4.9st-64bit" << endl;
#endif
#endif
