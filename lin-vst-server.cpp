...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR cmdlinexxx, int cmdshow)
{
    ...
    #ifdef EMBED
    #ifdef VST32SERVER
    cerr << "LinVst-X version 3.3-32bit" << endl;
    #else
    cerr << "LinVst-X version 3.3-64bit" << endl;    
    #endif
    #else
    #ifdef VST32SERVER
    cerr << "LinVst-X version 3.3st-32bit" << endl;
    #else
    cerr << "LinVst-X version 3.3st-64bit" << endl;    
    #endif    
    #endif
...
DWORD dwElapsed;
    ...
    remoteVSTServerInstance2[pidx]->m_plugin->dispatcher(remoteVSTServerInstance2[pidx]->m_plugin, effEditGetRect, 0, 0, &remoteVSTServerInstance2[pidx]->rect, 0); 
  	sched_yield();	  
    ...
    if(remoteVSTServerInstance2[pidx])
    {
    remoteVSTServerInstance2[pidx]->m_plugin->dispatcher(remoteVSTServerInstance2[pidx]->m_plugin, effEditClose, 0, 0, 0, 0);	
    sched_yield();	      
    if(timerval[pidx])
...
//#endif
	...
	if(remoteVSTServerInstance2[pidx])
    {		  
    remoteVSTServerInstance2[pidx]->m_plugin->dispatcher(remoteVSTServerInstance2[pidx]->m_plugin, effOpen, 0, 0, NULL, 0);
    sched_yield();	
    ...
    remoteVSTServerInstance2[pidx]->m_plugin->dispatcher(remoteVSTServerInstance2[pidx]->m_plugin, effClose, 0, 0, NULL, 0);
    sched_yield();	
...
*/
    ...
    getinstance = (VstEntry)GetProcAddress(remoteVSTServerInstance2[pidx]->libHandle, NEW_PLUGIN_ENTRY_POINT);

    if (!getinstance) {
    getinstance = (VstEntry)GetProcAddress(remoteVSTServerInstance2[pidx]->libHandle, OLD_PLUGIN_ENTRY_POINT);
    if (!getinstance) {
    cerr << "dssi-vst-server: ERROR: VST entrypoints \"" << NEW_PLUGIN_ENTRY_POINT << "\" or \""
                << OLD_PLUGIN_ENTRY_POINT << "\" not found in DLL \"" << libnamesync << "\"" << endl;   
    remoteVSTServerInstance2[pidx]->plugerr = 1;
    hcidx = 512000;
    sched_yield();	
    SetEvent(remoteVSTServerInstance2[pidx]->ghWriteEvent7);                
    ...
    if(remoteVSTServerInstance2[pidx]->m_plugin->magic != kEffectMagic)
    {
    ...
    remoteVSTServerInstance2[pidx]->m_plugin->resvd2 = (RemoteVSTServer *) remoteVSTServerInstance2[pidx];  
    hcidx = 512000;
    sched_yield();	
    SetEvent(remoteVSTServerInstance2[pidx]->ghWriteEvent7);   
    }
	...
	if(valt < plugincount)	
    remoteVSTServerInstance2[valt]->m_plugin->dispatcher(remoteVSTServerInstance2[valt]->m_plugin, effEditIdle, 0, 0, NULL, 0);    	
    }  
    }  
    break;
