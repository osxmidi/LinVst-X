...
public:
  ...
  virtual int getInputCount() {
    if (m_plugin[pidx])
      return m_plugin[pidx]->numInputs;
  }
  virtual int getOutputCount() {
    if (m_plugin[pidx])
      return m_plugin[pidx]->numOutputs;
  }
  virtual int getFlags() {
    if (m_plugin[pidx])
      return m_plugin[pidx]->flags;
  }
  virtual int getinitialDelay() {
    if (m_plugin[pidx])
      return m_plugin[pidx]->initialDelay;
  }
  virtual int getUID() {
    if (m_plugin[pidx])
      return m_plugin[pidx]->uniqueID;
  }
  virtual int getParameterCount() {
    if (m_plugin[pidx])
      return m_plugin[pidx]->numParams;
  }
  ...
  virtual int getProgramCount() {
    if (m_plugin[pidx])
      return m_plugin[pidx]->numPrograms;
  }
  ...
  //    virtual int         getInitialDelay() {return
  //    m_plugin[pidx]->initialDelay;} virtual int         getUniqueID() {
  //    return m_plugin[pidx]->uniqueID;} virtual int         getVersion() {
  //    return m_plugin[pidx]->version;}
...
RemoteVSTServer::RemoteVSTServer(std::string fileIdentifiers,
    ...
    : RemotePluginServer(fileIdentifiers), m_name(fallbackName), m_maker(""),
      ...
      ghWriteEvent7(0), pidx(0), plugerr(0), debugLevel(RemotePluginDebugNone) {
  ThreadHandle[0] = 0;
  ThreadHandle[1] = 0;
  ThreadHandle[2] = 0;
...
LRESULT CALLBACK RemoteVSTServer::realWndProc(HWND hWnd, UINT msg,
  ...
  case WM_TIMER:
    /*
             m_plugin[pidx]->dispatcher ( m_plugin[pidx], effEditIdle, 0, 0,
       NULL, 0);
            //    printf("timer %d\n", pidx);
    */
    break;
...
static VstIntPtr VESTIGECALLBACK hostCallback5(AEffect *plugin, VstInt32 opcode,
                                               VstInt32 index, VstIntPtr value,
                                               void *ptr, float opt) {
  VstIntPtr rv = 0;
...
std::string RemoteVSTServer::getMaker() {
  char buffer[512];
  memset(buffer, 0, sizeof(buffer));
  m_plugin[pidx]->dispatcher(m_plugin[pidx], effGetVendorString, 0, 0, buffer,
                             0);
  if (buffer[0])
    m_maker = buffer;
  return m_maker;
...
void RemoteVSTServer::EffectOpen() {
  ...
  m_plugin[pidx]->dispatcher(m_plugin[pidx], effSetSampleRate, 0, 0, NULL,
                             (float)44100);
...
void RemoteVSTServer::process(float **inputs, float **outputs,
  ...
  m_plugin[pidx]->processReplacing(m_plugin[pidx], inputs, outputs,
                                   sampleFrames);
...
void RemoteVSTServer::processdouble(double **inputs, double **outputs,
                                    int sampleFrames) {
  inProcessThread = true;
  m_plugin[pidx]->processDoubleReplacing(m_plugin[pidx], inputs, outputs,
                                         sampleFrames);
  inProcessThread = false;
}
...
bool RemoteVSTServer::setPrecision(int value) {
  ...
  retval = m_plugin[pidx]->dispatcher(m_plugin[pidx], effSetProcessPrecision, 0,
                                      value, 0, 0);
...
bool RemoteVSTServer::getOutProp(int index) {
  ...
  retval = m_plugin[pidx]->dispatcher(m_plugin[pidx], effGetOutputProperties,
                                      index, 0, &ptr, 0);
...
bool RemoteVSTServer::getInProp(int index) {
  ...
  retval = m_plugin[pidx]->dispatcher(m_plugin[pidx], effGetInputProperties,
                                      index, 0, &ptr, 0);
...
bool RemoteVSTServer::getMidiKey(int index) {
  ...
  retval = m_plugin[pidx]->dispatcher(m_plugin[pidx], effGetMidiKeyName, index,
                                      0, &ptr, 0);
...
bool RemoteVSTServer::getMidiProgName(int index) {
  ...
  retval = m_plugin[pidx]->dispatcher(m_plugin[pidx], effGetMidiProgramName,
                                      index, 0, &ptr, 0);
...
bool RemoteVSTServer::getMidiCurProg(int index) {
  ...
  retval = m_plugin[pidx]->dispatcher(m_plugin[pidx], effGetCurrentMidiProgram,
                                      index, 0, &ptr, 0);
  tryWrite(&m_shm2[FIXED_SHM_SIZE2 - sizeof(MidiProgramName)], &ptr,
           sizeof(MidiProgramName));
...
bool RemoteVSTServer::getMidiProgCat(int index) {
  ...
  retval = m_plugin[pidx]->dispatcher(m_plugin[pidx], effGetMidiProgramCategory,
                                      index, 0, &ptr, 0);
...
bool RemoteVSTServer::getMidiProgCh(int index) {
  ...
  retval = m_plugin[pidx]->dispatcher(m_plugin[pidx], effHasMidiProgramsChanged,
                                      index, 0, 0, 0);
...
bool RemoteVSTServer::setSpeaker() {
  ...
  retval = m_plugin[pidx]->dispatcher(m_plugin[pidx], effSetSpeakerArrangement,
                                      0, (VstIntPtr)&value, &ptr, 0);
...
bool RemoteVSTServer::getEffCanDo(std::string ptr) {
  if (m_plugin[pidx]->dispatcher(m_plugin[pidx], effCanDo, 0, 0,
                                 (char *)ptr.c_str(), 0))
    return true;
  else
    return false;
...
int RemoteVSTServer::effDoVoid2(int opcode, int index, int value, float opt) {
   ...
   */
  ret = m_plugin[pidx]->dispatcher(m_plugin[pidx], opcode, index, value, NULL,
                                   opt);
  return ret;
}
...
int RemoteVSTServer::getProgramNameIndexed(int p, char *name) {
  ...
  retval = m_plugin[pidx]->dispatcher(m_plugin[pidx], effGetProgramNameIndexed,
                                      p, 0, nameret, 0);
  strcpy(name, nameret);
  return retval;
}
...
int RemoteVSTServer::getShellName(char *name) {
  ...
  memset(nameret, 0, sizeof(nameret));
  retval = m_plugin[pidx]->dispatcher(m_plugin[pidx], effShellGetNextPlugin, 0,
                                      0, nameret, 0);
  strcpy(name, nameret);
  return retval;
}
...
void RemoteVSTServer::getChunk() {
  ...
  int sz = m_plugin[pidx]->dispatcher(m_plugin[pidx], effGetChunk, bnk_prg, 0,
                                      &chunkptr, 0);
...
void RemoteVSTServer::setChunk() {
  ...
  if (sz >= CHUNKSIZEMAX) {
    int bnk_prg = readIntring(&m_shmControl3->ringBuffer);
    void *ptr = chunkptr2;
    int r = m_plugin[pidx]->dispatcher(m_plugin[pidx], effSetChunk, bnk_prg, sz,
                                       ptr, 0);
    free(chunkptr2);
...
void RemoteVSTServer::canBeAutomated() {
  ...
  int r = m_plugin[pidx]->dispatcher(m_plugin[pidx], effCanBeAutomated, param,
                                     0, 0, 0);
  writeInt(&m_shm[FIXED_SHM_SIZE], r);
}
        ...
        while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
          ...
          case WM_SYNC: {
            ...
            if (remoteVSTServerInstance2[pidx]) {
              ...
              m_plugin[pidx]->dispatcher(m_plugin[pidx], effEditGetRect, 0, 0,
                                         &remoteVSTServerInstance2[pidx]->rect,
                                         0);
              sched_yield();
              m_plugin[pidx]->dispatcher(m_plugin[pidx], effEditOpen, 0, 0,
                                         hWndvst[pidx], 0);
              sched_yield();
              m_plugin[pidx]->dispatcher(m_plugin[pidx], effEditGetRect, 0, 0,
                                         &remoteVSTServerInstance2[pidx]->rect,
          ...
          case WM_SYNC2: {
            ...
            if (remoteVSTServerInstance2[pidx]) {
              m_plugin[pidx]->dispatcher(m_plugin[pidx], effEditClose, 0, 0, 0,
                                         0);
              sched_yield();
              if (timerval[pidx])
          ...
          case WM_SYNC4: {
            ...
            if (remoteVSTServerInstance2[pidx]) {
              m_plugin[pidx]->dispatcher(m_plugin[pidx], effOpen, 0, 0, NULL,
                                         0);
              sched_yield();
          ...
          case WM_SYNC5: {
            ...
            if (remoteVSTServerInstance2[pidx]) {
              if (remoteVSTServerInstance2[pidx]->effectrun == true)
                m_plugin[pidx]->dispatcher(m_plugin[pidx], effClose, 0, 0, NULL,
                                           0);
              sched_yield();
              if (remotelib[pidx])
          ...
          case WM_TIMER: {
            if (msg.wParam >= 6788888) {
              ...
              if (valt < plugincount)
                m_plugin[valt]->dispatcher(m_plugin[valt], effEditIdle, 0, 0,
                                           NULL, 0);
            }
          } break;
