...
public:
    ...
    virtual int         getInputCount() { if(m_plugin[pidx]) return m_plugin[pidx]->numInputs; }
    virtual int         getOutputCount() { if(m_plugin[pidx]) return m_plugin[pidx]->numOutputs; }
    virtual int         getFlags() { if(m_plugin[pidx]) return m_plugin[pidx]->flags; }
    virtual int         getinitialDelay() { if(m_plugin[pidx]) return m_plugin[pidx]->initialDelay; }
    virtual int         getUID() { if(m_plugin[pidx]) return m_plugin[pidx]->uniqueID; }
    virtual int         getParameterCount() { if(m_plugin[pidx]) return m_plugin[pidx]->numParams; }
    ...
    virtual int         getProgramCount() { if(m_plugin[pidx]) return m_plugin[pidx]->numPrograms; }
...
std::string RemoteVSTServer::getName()
{
      ...
      m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetEffectName, 0, 0, buffer, 0);
      if (buffer[0])
      m_name = buffer;
      return m_name;
}
...
std::string RemoteVSTServer::getMaker()
{
      char buffer[512];
      memset(buffer, 0, sizeof(buffer));
      m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetVendorString, 0, 0, buffer, 0);
      if (buffer[0])
      m_maker = buffer;
      return m_maker;
}
...
void RemoteVSTServer::EffectOpen()
{
    ...
    while(0 == PostThreadMessage(mainThreadId, WM_SYNC4, (WPARAM)pidx, (LPARAM) wname))
    sched_yield();
    dwWaitResult = WaitForSingleObject(ghWriteEvent4, 20000); 
    CloseHandle(ghWriteEvent4);
    sched_yield();
	
    m_plugin[pidx]->dispatcher( m_plugin[pidx], effMainsChanged, 0, 0, NULL, 0);
	
    m_plugin[pidx]->dispatcher( m_plugin[pidx], effSetBlockSize, 0, 1024, NULL, 0);

    m_plugin[pidx]->dispatcher( m_plugin[pidx], effSetSampleRate, 0, 0, NULL, (float)44100);
...
*/    	    
    ...
    {
    m_plugin[pidx]->flags |= effFlagsHasEditor;
    haveGui = true;
    wavesthread = 1;
    bufferwaves = buffer;	    
    }
...
*/	
    ...
    struct amessage
    {
        ...
        am.pcount = m_plugin[pidx]->numPrograms;
        am.parcount = m_plugin[pidx]->numParams;
        am.incount = m_plugin[pidx]->numInputs;
        am.outcount = m_plugin[pidx]->numOutputs;
        am.delay = m_plugin[pidx]->initialDelay;
#ifndef DOUBLEP
        am.flags = m_plugin[pidx]->flags;	
    ...
    m_plugin[pidx]->dispatcher( m_plugin[pidx], effMainsChanged, 0, 1, NULL, 0);		
...
void RemoteVSTServer::process(float **inputs, float **outputs, int sampleFrames)
{
    ...
    m_plugin[pidx]->processReplacing( m_plugin[pidx], inputs, outputs, sampleFrames);
    inProcessThread = false;
}
...
{
    inProcessThread = true;
    m_plugin[pidx]->processDoubleReplacing( m_plugin[pidx], inputs, outputs, sampleFrames);
    inProcessThread = false;
}
...
bool retval;
        ...
        retval = m_plugin[pidx]->dispatcher( m_plugin[pidx], effSetProcessPrecision, 0, value, 0, 0);
...
bool retval;
        ...
        retval = m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetOutputProperties, index, 0, &ptr, 0);
...
bool retval;
        ...
        retval = m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetInputProperties, index, 0, &ptr, 0);
...
bool retval;
        ...
        retval = m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetMidiKeyName, index, 0, &ptr, 0);
...
bool retval;
        ...
        retval = m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetMidiProgramName, index, 0, &ptr, 0);
...
bool retval;
        ...
        retval = m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetCurrentMidiProgram, index, 0, &ptr, 0);
        tryWrite(&m_shm2[FIXED_SHM_SIZE2 - sizeof(MidiProgramName)], &ptr, sizeof(MidiProgramName));
...
bool retval;
        ...
        retval = m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetMidiProgramCategory, index, 0, &ptr, 0);
...
bool retval;
        ...
        retval = m_plugin[pidx]->dispatcher( m_plugin[pidx], effHasMidiProgramsChanged, index, 0, 0, 0);
...
bool retval;
       ...
       retval = m_plugin[pidx]->dispatcher( m_plugin[pidx], effSetSpeakerArrangement, 0, (VstIntPtr)&value, &ptr, 0);
...
{
        if(m_plugin[pidx]->dispatcher( m_plugin[pidx], effCanDo, 0, 0, (char*)ptr.c_str(), 0))
        return true;
        else
        return false;
}
...
{
    return m_plugin[pidx]->dispatcher(m_plugin[pidx], opcode, 0, value, NULL, 0);
}
...
//        hostreaper = 1;
    ...
    m_plugin[pidx]->dispatcher( m_plugin[pidx], opcode, 0, 0, NULL, 0);
}
 ...
 */ 
    ret = m_plugin[pidx]->dispatcher( m_plugin[pidx], opcode, index, value, NULL, opt);    
    return ret;
}
...
std::string RemoteVSTServer::getEffString(int opcode, int index)
{
    ...
    m_plugin[pidx]->dispatcher( m_plugin[pidx], opcode, index, 0, name, 0);
    return name;
}
...
void RemoteVSTServer::setBufferSize(int sz)
{
    ...
    {
        m_plugin[pidx]->dispatcher( m_plugin[pidx], effMainsChanged, 0, 0, NULL, 0);
        m_plugin[pidx]->dispatcher( m_plugin[pidx], effSetBlockSize, 0, sz, NULL, 0);
        m_plugin[pidx]->dispatcher( m_plugin[pidx], effMainsChanged, 0, 1, NULL, 0);
        bufferSize = sz;
    }
...
void RemoteVSTServer::setSampleRate(int sr)
{
    ...
    {
        m_plugin[pidx]->dispatcher( m_plugin[pidx], effMainsChanged, 0, 0, NULL, 0);
        m_plugin[pidx]->dispatcher( m_plugin[pidx], effSetSampleRate, 0, 0, NULL, (float)sr);
        m_plugin[pidx]->dispatcher( m_plugin[pidx], effMainsChanged, 0, 1, NULL, 0);
        sampleRate = sr;
    }
...
std::string RemoteVSTServer::getParameterName(int p)
{
    ...
    m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetParamName, p, 0, name, 0);
    return name;
}
...
std::string RemoteVSTServer::getParameterLabel(int p)
{
    ...
    m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetParamLabel, p, 0, name, 0);
    return name;
}
...
std::string RemoteVSTServer::getParameterDisplay(int p)
{
    ...
    m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetParamDisplay, p, 0, name, 0);
    return name;
}

void RemoteVSTServer::setParameter(int p, float v)
{
    m_plugin[pidx]->setParameter( m_plugin[pidx], p, v);
}

float RemoteVSTServer::getParameter(int p)
{
    return m_plugin[pidx]->getParameter( m_plugin[pidx], p);
}

void RemoteVSTServer::getParameters(int p0, int pn, float *v)
{
    for (int i = p0; i <= pn; ++i)
        v[i - p0] = m_plugin[pidx]->getParameter( m_plugin[pidx], i);
}
...
int RemoteVSTServer::getProgramNameIndexed(int p, char *name)
{
    ...
    retval = m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetProgramNameIndexed, p, 0, nameret, 0);
    strcpy(name, nameret); 
    return retval;
}
...
RemoteVSTServer::getProgramName()
{
    ...
    m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetProgramName, 0, 0, name, 0);
    return name;
}
...
char nameret[512];
    ...
    memset(nameret, 0, sizeof(nameret));
    retval = m_plugin[pidx]->dispatcher( m_plugin[pidx], effShellGetNextPlugin, 0, 0, nameret, 0);
    strcpy(name, nameret); 
    return retval;
}
#endif
...
*/
    ...
    if (p < m_plugin[pidx]->numPrograms)
        m_plugin[pidx]->dispatcher( m_plugin[pidx], effSetProgram, 0, p, 0, 0);        
}
...
int RemoteVSTServer::processVstEvents()
{
    ...
    m_plugin[pidx]->dispatcher( m_plugin[pidx], effProcessEvents, 0, 0, evptr, 0);
...
void RemoteVSTServer::getChunk()
{
    ...
    int sz = m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetChunk, bnk_prg, 0, &chunkptr, 0);
...
else
{
    ...
    void *ptr;
    int bnk_prg = readIntring(&m_shmControl3->ringBuffer);
    int sz = m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetChunk, bnk_prg, 0, &ptr, 0);
    if(sz < CHUNKSIZEMAX)
    ...
    if(sz >= CHUNKSIZEMAX)
{
    int bnk_prg = readIntring(&m_shmControl3->ringBuffer);
    void *ptr = chunkptr2;
    int r = m_plugin[pidx]->dispatcher( m_plugin[pidx], effSetChunk, bnk_prg, sz, ptr, 0);
    free(chunkptr2);
...
void RemoteVSTServer::canBeAutomated()
{
    ...
    int r = m_plugin[pidx]->dispatcher( m_plugin[pidx], effCanBeAutomated, param, 0, 0, 0);
    writeInt(&m_shm[FIXED_SHM_SIZE], r);
}

void RemoteVSTServer::getProgram()
{
    int r = m_plugin[pidx]->dispatcher( m_plugin[pidx], effGetProgram, 0, 0, 0, 0);
    writeInt(&m_shm[FIXED_SHM_SIZE], r);
}
...
///      ExitThread(0);
  ...
  //    ExitThread(0);
      ...
      if(m_plugin[idx]->flags & effFlagsHasEditor)
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR cmdlinexxx, int cmdshow)
{
 ...
 //   while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE))
    ...
    m_plugin[pidx]->dispatcher(m_plugin[pidx], effEditGetRect, 0, 0, &remoteVSTServerInstance2[pidx]->rect, 0); 
  	sched_yield();	  
    m_plugin[pidx]->dispatcher(m_plugin[pidx], effEditOpen, 0, 0, hWndvst[pidx], 0);   
    sched_yield();	  
    m_plugin[pidx]->dispatcher(m_plugin[pidx], effEditGetRect, 0, 0, &remoteVSTServerInstance2[pidx]->rect, 0);
 	sched_yield();	
    ...
    if(remoteVSTServerInstance2[pidx])
    {
    m_plugin[pidx]->dispatcher(m_plugin[pidx], effEditClose, 0, 0, 0, 0);	
    sched_yield();	      
    if(timerval[pidx])
    ...
    m_plugin[pidx]->dispatcher( m_plugin[pidx], effClose, 0, 0, NULL, 0);
    sched_yield();	
    if(remotelib[pidx])
...
*/
    ...
    m_plugin[pidx] = getinstance(hostCallback3);
    if (!m_plugin[pidx])
    {
    ...
    if(m_plugin[pidx]->magic != kEffectMagic)
    {
    cerr << "dssi-vst-server: ERROR: Not a VST plugin in DLL \"" << libnamesync << "\"" << endl;
    remoteVSTServerInstance2[pidx]->plugerr = 1;
    hcidx = 512000;
    sched_yield();	
    SetEvent(remoteVSTServerInstance2[pidx]->ghWriteEvent7);                  
    ...
    if (!(m_plugin[pidx]->flags & effFlagsCanReplacing))
    {
    cerr << "dssi-vst-server: ERROR: Not a VST plugin in DLL \"" << libnamesync << "\"" << endl;
    remoteVSTServerInstance2[pidx]->plugerr = 1;
    hcidx = 512000;
    ...
    m_plugin[pidx]->resvd2 = (RemoteVSTServer *) remoteVSTServerInstance2[pidx];  
    hcidx = 512000;
    sched_yield();	
    SetEvent(remoteVSTServerInstance2[pidx]->ghWriteEvent7);   
    }
    ...
    *sptr = 747;
    ...
    getinstance = (VstEntry)GetProcAddress(remotelib[plugincount], NEW_PLUGIN_ENTRY_POINT);

    if (!getinstance) {
    getinstance = (VstEntry)GetProcAddress(remotelib[plugincount], OLD_PLUGIN_ENTRY_POINT);
    if (!getinstance) {
